...
class ProteomicsRunServiceImpl extends ServiceBase implements ProteomicsRunService {
  ...
  public ProteomicsRun createProteomicsRun(final String userGridId, final String folderId, final ProteomicsRun run, final String mzxmlFileId,
      final String sampleId, @Nullable final String sourceFileId) {
    ...
    if(!StringUtils.hasText(mzxmlFile.getName())) {
    ...
    if(sourceFileId != null) {
      ...
      if(!StringUtils.hasText(sourceFile.getName())) {
        final String name = getMessageSource()
            .getMessage(MessageConstants.PROTEOMICS_RUN_SOURCE_NAME, runName, sourceFile.getFileType().getExtension());
