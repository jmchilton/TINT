...
public class SftpSubsystem implements Command, Runnable, SessionAware, FileSystemAware {
  ...
  protected abstract static class Handle {
    ...
    public Handle(final SshFile file) {
      this.file = file;
    }
  ...
  protected static class DirectoryHandle extends Handle {
    ...
    public DirectoryHandle(final SshFile file) {
      super(file);
    }
  ...
  protected static class FileHandle extends Handle {
    ...
    public FileHandle(final SshFile sshFile, final int flags) {
      super(sshFile);
      this.flags = flags;
    }
    ...
    public int read(final byte[] data, final long offset) throws IOException {
    ...
    public void write(final byte[] data, final long offset) throws IOException {
  ...
  public void setFileSystemView(final FileSystemView view) {
    this.root = view;
  }
  ...
  public void run() {
    ...
    try {
      ...
      while(true) {
        ...
        final Buffer buffer = new Buffer(length + 4);
        buffer.putInt(length);
        int nb = length;
        while(nb > 0) {
          final int l = dis.read(buffer.array(), buffer.wpos(), nb);
          if(l < 0) {
            throw new IllegalArgumentException();
          }
      ...
      if(handles != null) {
        for(final Map.Entry<String, Handle> entry : handles.entrySet()) {
          final Handle handle = entry.getValue();
          try {
            handle.close();
          } catch(final IOException ioe) {
  ...
  protected void process(final Buffer buffer) throws IOException {
    ...
    case SSH_FXP_OPEN: {
      if(session.getFactoryManager().getProperties() != null) {
        final String maxHandlesString = session.getFactoryManager().getProperties().get(MAX_OPEN_HANDLES_PER_SESSION);
        if(maxHandlesString != null) {
          final int maxHandleCount = Integer.parseInt(maxHandlesString);
      ...
      if(version <= 4) {
        ...
        final int pflags = buffer.getInt();
        // attrs
        try {
          final SshFile file = resolveFile(path);
          if(file.doesExist()) {
          ...
          final String acc = ((pflags & (SSH_FXF_READ | SSH_FXF_WRITE)) != 0 ? "r" : "") + ((pflags & SSH_FXF_WRITE) != 0 ? "w" : "");
    ...
    case SSH_FXP_CLOSE: {
      final String handle = buffer.getString();
      try {
    ...
    case SSH_FXP_READ: {
      ...
      try {
        final Handle p = handles.get(handle);
        if(!(p instanceof FileHandle)) {
          sendStatus(id, SSH_FX_INVALID_HANDLE, handle);
        } else {
          final FileHandle fh = (FileHandle) p;
          final byte[] b = new byte[Math.min(len, 1024 * 32)];
          len = fh.read(b, offset);
          if(len >= 0) {
            final Buffer buf = new Buffer(len + 5);
    ...
    case SSH_FXP_WRITE: {
      ...
      try {
        ...
        if(!(p instanceof FileHandle)) {
          ...
          final SshFile sshFile = fh.getFile();
    ...
    case SSH_FXP_STAT: {
      final String path = buffer.getString();
      try {
        final SshFile p = resolveFile(path);
        sendAttrs(id, p);
    ...
    case SSH_FXP_RENAME: {
      ...
      try {
        final SshFile o = resolveFile(oldPath);
        final SshFile n = resolveFile(newPath);
        if(!o.doesExist()) {
    ...
    case SSH_FXP_FSETSTAT: {
      ...
      try {
        ...
        if(p == null) {
          ...
          final SshFile file = p.file;
          setStat(id, buffer, file);
        }
      } catch(final FileNotFoundException e) {
        sendStatus(id, SSH_FX_NO_SUCH_FILE, e.getMessage());
      } catch(final IOException e) {
  ...
  protected void setStat(final int id, final Buffer buffer, final SshFile file) throws IOException {
    final int flags = buffer.getInt();
    log.error(String.format("setstat called %d", flags));
    if(version > 3) {
      ...
      if((flags & SSH_FILEXFER_ATTR_MODIFYTIME) != 0) {
        long modifiedTime = buffer.getLong();
      ...
      final int SSH_FILEXFER_ATTR_UIDGID = 2;
      if((flags & SSH_FILEXFER_ATTR_SIZE) != 0) {
        buffer.getLong(); // SIZE
      } 
  ...
  protected void sendHandle(final int id, final String handle) throws IOException {
  ...
  protected void sendAttrs(final int id, final SshFile file) throws IOException {
    final Buffer buffer = new Buffer();
  ...
  protected void sendAttrs(final int id, final SshFile file, final int flags) throws IOException {
    final Buffer buffer = new Buffer();
    buffer.putByte((byte) SSH_FXP_ATTRS);
    buffer.putInt(id);
  ...
  protected void sendName(final int id, final Collection<SshFile> files) throws IOException {
    final Buffer buffer = new Buffer();
    buffer.putByte((byte) SSH_FXP_NAME);
    buffer.putInt(id);
    buffer.putInt(files.size());
    for(final SshFile f : files) {
  ...
  private String getLongName(final SshFile f) {
    ...
    final long length = f.getSize();
    final String lengthString = String.format("%1$8s", length);
  ...
  protected void writeAttrs(final Buffer buffer, final SshFile file) throws IOException {
  ...
  protected void sendStatus(final int id, final int substatus, final String msg) throws IOException {
    sendStatus(id, substatus, msg, "");
  }

  protected void sendStatus(final int id, final int substatus, final String msg, final String lang) throws IOException {
    final Buffer buffer = new Buffer();
  ...
  protected void send(final Buffer buffer) throws IOException {
  ...
  private SshFile resolveFile(final String path) {
  ...
  private static String getUnixDate(final long millis) {
    ...
    final long sixMonth = 15811200000L; // 183L * 24L * 60L * 60L * 1000L;
