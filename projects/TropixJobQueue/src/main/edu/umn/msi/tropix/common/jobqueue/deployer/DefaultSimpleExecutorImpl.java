/********************************************************************************
 * Copyright (c) 2009 Regents of the University of Minnesota
 *
 * This Software was written at the Minnesota Supercomputing Institute
 * http://msi.umn.edu
 *
 * All rights reserved. The following statement of license applies
 * only to this file, and and not to the other files distributed with it
 * or derived therefrom.  This file is made available under the terms of
 * the Eclipse Public License v1.0 which accompanies this distribution,
 * and is available at http://www.eclipse.org/legal/epl-v10.html
 *
 * Contributors:
 * Minnesota Supercomputing Institute - initial API and implementation
 *******************************************************************************/

package edu.umn.msi.tropix.common.jobqueue.deployer;

import java.io.File;

import com.google.common.collect.Lists;

import edu.umn.msi.tropix.common.execution.process.Process;
import edu.umn.msi.tropix.common.execution.process.ProcessConfiguration;
import edu.umn.msi.tropix.common.execution.process.ProcessFactory;
import edu.umn.msi.tropix.common.execution.process.Processes;

class DefaultSimpleExecutorImpl implements SimpleExecutor {
  private final ProcessFactory processFactory;

  DefaultSimpleExecutorImpl() {
    this(Processes.getDefaultFactory());
  }

  DefaultSimpleExecutorImpl(final ProcessFactory processFactory) {
    this.processFactory = processFactory;
  }

  public int execute(final String application, final String... arguments) {
    return executeInDirectory(null, application, arguments);
  }

  public int executeInDirectory(final File directory, final String application, final String... arguments) {
    final ProcessConfiguration processConfiguration = new ProcessConfiguration();
    if(directory != null) {
      processConfiguration.setDirectory(directory);
    }
    processConfiguration.setApplication(application);
    processConfiguration.setArguments(Lists.newArrayList(arguments));
    return execute(processConfiguration);
  }

  public int execute(final ProcessConfiguration processConfiguration) {
    final Process process = processFactory.createProcess(processConfiguration);
    int returnVal;
    try {
      returnVal = process.waitFor();
    } catch(final InterruptedException e) {
      throw new RuntimeException("Inerrupted while waiting for process " + process);
    }
    return returnVal;
  }

}
